    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <aside className="fixed inset-y-0 left-0 z-30 w-64 border-r bg-white">
        <div className="flex h-16 items-center gap-2 border-b px-6">
          <img src="/logo.svg" alt="Starlight" className="h-8 w-8 rounded-lg" />
...
function DashboardIcon({ active }: { active: boolean }) {
  return (
    <svg
      className={`h-5 w-5 ${active ? "text-brand-600" : "text-gray-400"}`}
      fill="none"
      viewBox="0 0 24 24"
      strokeWidth={1.5}
      stroke="currentColor"
...
function RatingsIcon({ active }: { active: boolean }) {
  return (
    <svg
      className={`h-5 w-5 ${active ? "text-brand-600" : "text-gray-400"}`}
      fill="none"
