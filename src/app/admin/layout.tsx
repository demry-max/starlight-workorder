...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { href: '/admin/dashboard', label: t('admin.nav.dashboard'), icon: DashboardIcon },
    { href: '/admin/workorders', label: t('admin.nav.workorders'), icon: OrdersIcon },
  ];
...
function DashboardIcon({ active }: { active: boolean }) {
  return (
    <svg className={`h-5 w-5 ${active ? 'text-brand-600' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
